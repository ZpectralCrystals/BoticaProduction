...
\restrict D6bwGOfH22dCKkb2sKcb6Geb6gvHVYt3nobc88aUCXecVTM6ZifzMxbhGQhoQkb
...
--
-- Name: fn_bot_lotes_block_expired_active(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_bot_lotes_block_expired_active() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: fn_bot_lotes_sync_producto_stock(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_bot_lotes_sync_producto_stock() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: fn_bot_productos_stock_activo_lotes(integer); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_bot_productos_stock_activo_lotes(p_producto_id integer) RETURNS integer
    LANGUAGE sql STABLE
    AS $$
...
--
-- Name: fn_bot_productos_validate_stock_lotes(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_bot_productos_validate_stock_lotes() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
-- Name: fn_bot_sync_producto_stock_from_lotes(integer); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_bot_sync_producto_stock_from_lotes(p_producto_id integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_stock INTEGER;
...
CREATE TABLE public.bot_almacenes (
    ...
    CONSTRAINT chk_almacen_tipo CHECK (((ctipo_almacen)::text = ANY (ARRAY[('DISPONIBLE'::character varying)::text, ('CUARENTENA'::character varying)::text, ('DEVOLUCION_CLIENTE'::character varying)::text, ('DEVOLUCION_PROVEEDOR'::character varying)::text, ('BAJA'::character varying)::text, ('PROCEDIMIENTOS'::character varying)::text, ('CONTROL_ESPECIAL'::character varying)::text])))
);
...
CREATE TABLE public.bot_alquileres (
    ...
    CONSTRAINT bot_alquileres_cperiodo_check CHECK (((cperiodo)::text = ANY (ARRAY[('Diario'::character varying)::text, ('Semanal'::character varying)::text, ('Mensual'::character varying)::text])))
);
...
CREATE TABLE public.bot_caja_movimientos (
    ...
    tcreado timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_bot_caja_mov_metodo CHECK (((cmetodo_pago)::text = ANY (ARRAY[('EFECTIVO'::character varying)::text, ('TARJETA'::character varying)::text, ('TRANSFERENCIA'::character varying)::text, ('YAPE'::character varying)::text, ('PLIN'::character varying)::text, ('OTRO'::character varying)::text]))),
    CONSTRAINT chk_bot_caja_mov_monto_pos CHECK ((nmonto > (0)::numeric)),
    CONSTRAINT chk_bot_caja_mov_tipo CHECK (((ctipo)::text = ANY (ARRAY[('INGRESO'::character varying)::text, ('EGRESO'::character varying)::text, ('PAGO_FACTURA'::character varying)::text, ('GASTO'::character varying)::text, ('VENTA'::character varying)::text, ('DEVOLUCION'::character varying)::text])))
);
...
CREATE TABLE public.bot_citas (
    ...
    tcreado timestamp without time zone DEFAULT now(),
    nmedico_id integer,
    CONSTRAINT bot_citas_cestado_check CHECK (((cestado)::text = ANY (ARRAY[('Confirmada'::character varying)::text, ('En espera'::character varying)::text, ('Atendida'::character varying)::text])))
);
...
CREATE TABLE public.bot_compras (
    ...
    CONSTRAINT chk_bot_compras_tipo_pago CHECK (((ctipo_pago)::text = ANY (ARRAY[('CONTADO'::character varying)::text, ('CREDITO'::character varying)::text])))
);
...
CREATE TABLE public.bot_cuentas_por_pagar (
    ...
    tcreado timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    tmodifi timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_bot_cxp_estado CHECK (((cestado)::text = ANY (ARRAY[('PENDIENTE'::character varying)::text, ('PARCIAL'::character varying)::text, ('PAGADA'::character varying)::text, ('ANULADA'::character varying)::text]))),
...
CREATE TABLE public.bot_locales (
    ...
    CONSTRAINT chk_locales_tipo CHECK (((ctipo_local)::text = ANY (ARRAY[('BOTICA'::character varying)::text, ('CLINICA'::character varying)::text, ('OTRO'::character varying)::text])))
);
...
CREATE TABLE public.bot_lotes (
    ...
    CONSTRAINT chk_bot_lotes_estado CHECK (((cestado)::text = ANY (ARRAY[('ACTIVO'::character varying)::text, ('AGOTADO'::character varying)::text, ('VENCIDO'::character varying)::text]))),
...
CREATE TABLE public.bot_movimientos_almacen (
    ...
    CONSTRAINT chk_mov_tipo CHECK (((ctipo_movimiento)::text = ANY (ARRAY[('COMPRA'::character varying)::text, ('VENTA'::character varying)::text, ('CONSUMO_CLINICO'::character varying)::text, ('TRASLADO'::character varying)::text, ('DEVOLUCION_CLIENTE'::character varying)::text, ('DEVOLUCION_PROVEEDOR'::character varying)::text, ('BAJA'::character varying)::text, ('AJUSTE'::character varying)::text])))
);
...
CREATE TABLE public.bot_producto_precios (
    ...
    CONSTRAINT chk_bot_producto_precios_nombre CHECK (((cnombre)::text = ANY (ARRAY[('PRECIO_1'::character varying)::text, ('PRECIO_2'::character varying)::text, ('PRECIO_3'::character varying)::text])))
);
...
CREATE TABLE public.bot_productos (
    ...
    CONSTRAINT bot_productos_crotacion_check CHECK (((crotacion)::text = ANY (ARRAY[('Alta'::character varying)::text, ('Media'::character varying)::text, ('Baja'::character varying)::text]))),
    CONSTRAINT bot_productos_ctipo_producto_check CHECK (((ctipo_producto)::text = ANY (ARRAY[('MEDICAMENTO'::character varying)::text, ('NO_MEDICAMENTO'::character varying)::text]))),
...
CREATE TABLE public.bot_proveedores (
    ...
    CONSTRAINT chk_bot_proveedores_estado_activo_inactivo CHECK ((cestado = ANY (ARRAY['A'::bpchar, 'I'::bpchar])))
);
...
CREATE TABLE public.bot_transferencias (
    ...
    cestado character(1) DEFAULT 'A'::bpchar NOT NULL,
    tcreado timestamp without time zone DEFAULT now(),
    CONSTRAINT bot_transferencias_ctipo_check CHECK (((ctipo)::text = ANY (ARRAY[('Transferencia'::character varying)::text, ('Muestra'::character varying)::text, ('Donacion'::character varying)::text, ('Regalo'::character varying)::text, ('Merma'::character varying)::text, ('Entrada'::character varying)::text])))
);
...
CREATE TABLE public.bot_ventas (
    ...
    CONSTRAINT bot_ventas_cmetpago_check CHECK (((cmetpago)::text = ANY (ARRAY[('Efectivo'::character varying)::text, ('Yape'::character varying)::text, ('Mixto'::character varying)::text]))),
...
CREATE TABLE public.bot_ventas_det (
    ...
    CONSTRAINT bot_ventas_det_ctipo_check CHECK (((ctipo)::text = ANY (ARRAY[('Producto'::character varying)::text, ('Servicio'::character varying)::text])))
);
...
--
-- Name: vw_bot_proveedores_invalidos; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.vw_bot_proveedores_invalidos AS
 WITH normalized AS (
         SELECT p.nid,
            COALESCE(p.cruc, ''::character varying) AS cruc,
            COALESCE(p.cnombre, ''::character varying) AS cnombre,
            COALESCE(p.ccontacto, ''::character varying) AS ccontacto,
            COALESCE(p.ctelefono, ''::character varying) AS ctelefono,
            COALESCE(p.cemail, ''::character varying) AS cemail,
            COALESCE(p.cdireccion, ''::text) AS cdireccion,
            COALESCE(p.cestado, 'A'::bpchar) AS cestado,
            regexp_replace((COALESCE(p.cruc, ''::character varying))::text, '\D'::text, ''::text, 'g'::text) AS ruc_normalizado,
            regexp_replace((COALESCE(p.ctelefono, ''::character varying))::text, '\D'::text, ''::text, 'g'::text) AS telefono_normalizado
           FROM public.bot_proveedores p
        )
 SELECT normalized.nid,
    normalized.cruc,
    normalized.cnombre,
    normalized.ccontacto,
    normalized.ctelefono,
    normalized.cemail,
    normalized.cdireccion,
    normalized.cestado,
    array_to_string(array_remove(ARRAY[
        CASE
            WHEN (NULLIF(normalized.ruc_normalizado, ''::text) IS NULL) THEN 'RUC vacío'::text
            ELSE NULL::text
        END,
        CASE
            WHEN ((NULLIF(normalized.ruc_normalizado, ''::text) IS NOT NULL) AND (length(normalized.ruc_normalizado) <> 11)) THEN 'RUC sin 11 dígitos'::text
            ELSE NULL::text
        END,
        CASE
            WHEN (NULLIF(btrim((normalized.cnombre)::text), ''::text) IS NULL) THEN 'Razón social vacía'::text
            ELSE NULL::text
        END,
        CASE
            WHEN (NULLIF(btrim((normalized.ccontacto)::text), ''::text) IS NULL) THEN 'Contacto vacío'::text
            ELSE NULL::text
        END,
        CASE
            WHEN (NULLIF(btrim((normalized.ctelefono)::text), ''::text) IS NULL) THEN 'Teléfono vacío'::text
            ELSE NULL::text
        END,
        CASE
            WHEN ((NULLIF(btrim((normalized.ctelefono)::text), ''::text) IS NOT NULL) AND (length(normalized.telefono_normalizado) < 6)) THEN 'Teléfono con menos de 6 dígitos'::text
            ELSE NULL::text
        END,
        CASE
            WHEN ((NULLIF(btrim((normalized.cestado)::text), ''::text) IS NULL) OR (normalized.cestado <> ALL (ARRAY['A'::bpchar, 'I'::bpchar]))) THEN 'Estado inválido'::text
            ELSE NULL::text
        END], NULL::text), ' | '::text) AS observaciones
   FROM normalized
  WHERE ((NULLIF(normalized.ruc_normalizado, ''::text) IS NULL) OR (length(normalized.ruc_normalizado) <> 11) OR (NULLIF(btrim((normalized.cnombre)::text), ''::text) IS NULL) OR (NULLIF(btrim((normalized.ccontacto)::text), ''::text) IS NULL) OR (NULLIF(btrim((normalized.ctelefono)::text), ''::text) IS NULL) OR (length(normalized.telefono_normalizado) < 6) OR (NULLIF(btrim((normalized.cestado)::text), ''::text) IS NULL) OR (normalized.cestado <> ALL (ARRAY['A'::bpchar, 'I'::bpchar])));


--
-- Name: vw_bot_proveedores_ruc_duplicados; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.vw_bot_proveedores_ruc_duplicados AS
 WITH normalized AS (
         SELECT bot_proveedores.nid,
            regexp_replace((COALESCE(bot_proveedores.cruc, ''::character varying))::text, '\D'::text, ''::text, 'g'::text) AS ruc_normalizado
           FROM public.bot_proveedores
        )
 SELECT normalized.ruc_normalizado AS ruc,
    count(*) AS total_registros,
    string_agg((normalized.nid)::text, ', '::text ORDER BY normalized.nid) AS proveedores_ids
   FROM normalized
  WHERE (NULLIF(normalized.ruc_normalizado, ''::text) IS NOT NULL)
  GROUP BY normalized.ruc_normalizado
 HAVING (count(*) > 1);
...
--
-- Name: idx_bot_caja_cerrado_por; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_caja_cerrado_por ON public.bot_caja USING btree (ncerrado_por_id) WHERE (ncerrado_por_id IS NOT NULL);
...
--
-- Name: idx_bot_caja_mov_usuario; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_caja_mov_usuario ON public.bot_caja_movimientos USING btree (nusuario_id) WHERE (nusuario_id IS NOT NULL);


--
-- Name: idx_bot_caja_usuario; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_caja_usuario ON public.bot_caja USING btree (nusuario_id) WHERE (nusuario_id IS NOT NULL);
...
--
-- Name: idx_bot_compras_det_compra; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_compras_det_compra ON public.bot_compras_det USING btree (ncompra_id);


--
-- Name: idx_bot_compras_det_producto; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_compras_det_producto ON public.bot_compras_det USING btree (nproducto_id);


--
-- Name: idx_bot_compras_proveedor; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_compras_proveedor ON public.bot_compras USING btree (nproveedor_id);
...
--
-- Name: idx_bot_compras_usuario; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_compras_usuario ON public.bot_compras USING btree (nusuario_id) WHERE (nusuario_id IS NOT NULL);
...
CREATE INDEX idx_bot_cxp_pendientes ON public.bot_cuentas_por_pagar USING btree (tfecha_vencimiento) WHERE ((cestado)::text = ANY (ARRAY[('PENDIENTE'::character varying)::text, ('PARCIAL'::character varying)::text]));
...
--
-- Name: idx_bot_kardex_producto_fecha; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_kardex_producto_fecha ON public.bot_kardex USING btree (nproducto_id, tcreado DESC);
...
--
-- Name: idx_bot_productos_proveedor; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_productos_proveedor ON public.bot_productos USING btree (nproveedor_id) WHERE (nproveedor_id IS NOT NULL);
...
--
-- Name: idx_bot_ventas_det_producto; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_ventas_det_producto ON public.bot_ventas_det USING btree (nproducto_id) WHERE (nproducto_id IS NOT NULL);


--
-- Name: idx_bot_ventas_det_servicio; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_ventas_det_servicio ON public.bot_ventas_det USING btree (nservicio_id) WHERE (nservicio_id IS NOT NULL);


--
-- Name: idx_bot_ventas_det_venta; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_ventas_det_venta ON public.bot_ventas_det USING btree (nventa_id);


--
-- Name: idx_bot_ventas_usuario; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_bot_ventas_usuario ON public.bot_ventas USING btree (nusuario_id) WHERE (nusuario_id IS NOT NULL);
...
--
-- Name: ux_bot_proveedores_ruc_normalizado; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_bot_proveedores_ruc_normalizado ON public.bot_proveedores USING btree (regexp_replace((COALESCE(cruc, ''::character varying))::text, '\D'::text, ''::text, 'g'::text)) WHERE (NULLIF(regexp_replace((COALESCE(cruc, ''::character varying))::text, '\D'::text, ''::text, 'g'::text), ''::text) IS NOT NULL);
...
--
-- Name: bot_lotes trg_bot_lotes_block_expired_active; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bot_lotes_block_expired_active BEFORE INSERT OR UPDATE OF cestado, ncantidad, dfechavencimiento ON public.bot_lotes FOR EACH ROW EXECUTE FUNCTION public.fn_bot_lotes_block_expired_active();


--
-- Name: bot_lotes trg_bot_lotes_sync_producto_stock; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bot_lotes_sync_producto_stock AFTER INSERT OR DELETE OR UPDATE OF nproducto_id, ncantidad, cestado ON public.bot_lotes FOR EACH ROW EXECUTE FUNCTION public.fn_bot_lotes_sync_producto_stock();
...
--
-- Name: bot_productos trg_bot_productos_validate_stock_lotes; Type: TRIGGER; Schema: public; Owner: -
--

CREATE CONSTRAINT TRIGGER trg_bot_productos_validate_stock_lotes AFTER INSERT OR UPDATE OF nstock, lrequiere_lote, cestado ON public.bot_productos DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION public.fn_bot_productos_validate_stock_lotes();
...
\unrestrict D6bwGOfH22dCKkb2sKcb6Geb6gvHVYt3nobc88aUCXecVTM6ZifzMxbhGQhoQkb
